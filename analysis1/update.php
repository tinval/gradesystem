...
if ($input['action'] === 'edit') {
    $stmt = $conn->prepare('UPDATE ' . $config['database']['tablename1']  . ' SET assistent=? WHERE leginr="' . $input['leginr']. '"');
    $stmt->bind_param('s', $input['assi']);
    $stmt->execute();
    $stmt = $conn->prepare('UPDATE ' . $config['database']['tablename1'] . ' SET ' . $entry . '=? WHERE leginr="' . $input['leginr']. '"');
