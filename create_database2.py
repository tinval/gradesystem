...
csv_data = csv.reader(open(cfg['edoz_file2']), delimiter='\t')
not_header = False
new_students = 0
total_students = 0
for data in csv_data:
    if not_header:
        total_students += 1
        query='INSERT INTO ' + cfg['database']['tablename2'] + ' (leginr, vorname, nachname, kurz) VALUES ("{}","{}", "{}", "{}");'.format(data[3], data[1], data[0], data[22].split('@')[0])
        try:
            cursor.execute(query)
            print(query)
            new_students += 1
